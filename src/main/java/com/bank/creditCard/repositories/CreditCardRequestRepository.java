...
public interface CreditCardRequestRepository
  extends
   JpaRepository<CardRequestDetails, String> {
 @Query(value = "SELECT * FROM CARD_REQUEST_DETAILS C WHERE C.REQUEST_TYPE=:requestType AND C.STATUS= :status ", nativeQuery = true)
 public List<CardRequestDetails> findRequestDetailsByRequestTypeAndStatus(
   @Param("requestType") Short requestType, @Param("status") Short status);
 @Modifying
 ...
 public void updateUserCardIdAndStatusToTheRequest(
 ...
 public void updateStatusToTheRequest(@Param("status") Short status,
