...
public class SchedularService {
	...
	public void doTask() {
		...
		List<CardRequestDetails> cardRequestDetailsList = cardRequestRepository.findRequestDetailsByRequestTypeAndStatus(Constants.REQUEST_TYPE_NEW_CUSTOMER_NEW_CARD, Constants.REQUEST_STATUS_APPROVED);
	...
	private void generateCreditCardsAndMapToUser(List<CardRequestDetails> cardRequestDetailsList,
			List<CustomerDetails> customerDetailsList, List<CreditCardName> creditCardNamesList) {
	...
	public void rewardPointsSchedularTask() {
		...
		if (transactionDetailsList != null && !transactionDetailsList.isEmpty()) {
			...
			List<Long> creditCardIds = transactionDetailsList.stream().map(TransactionDetails::getCardId)
			...
			Map<Long, List<TransactionDetails>> tranctionDetailsMapByCardId = transactionDetailsList.stream().filter( trans -> (trans.getAmount().compareTo(hundred)) > -1 )
			...
			for (Map.Entry<Long, List<TransactionDetails>> entry : tranctionDetailsMapByCardId.entrySet()) {
				...
				if(userCardDetails != null) {
					cardNameDetails = creditCardNamesList.stream().filter(card -> userCardDetails.getCardName().equals(card.getCardName())).findAny().orElse(null);
				}
				
				try {
					rewardPointsSchedularService.processTransactions( transactions, userCardDetails, cardNameDetails);
					logger.info("RewardPointPorcess completed for card: {}", userCardDetails.getUserCardId());
				} catch (Exception e) {
					logger.error("RewardPointPorcess failed: {}", userCardDetails.getUserCardId(), e);
				}
	...
	public void deleteUserSchedularTask() {
		...
		List<CardRequestDetails> cardRequestDetailsList =  cardRequestRepository.findAllByStatus(Constants.REQUEST_STATUS_REJECTED);
		...
		if(!customerIds.isEmpty()) {
			...
			List<CreditCardDetails> cardDetails = creditCardDetailsRepository.findCardsByUserIds(customerIds);
