...
public class CardTransactionService {
 ...
 /**
  * This method is used to save the Credit Card transaction in
  * {@link TransactionDetails}
  * <p>
  * This method will validate {@link TransactionInputDetails} and save the
  * {@link TransactionDetails} with transaction status. If the transaction
  * status is success the, service will calculate the Outstanding amount and
  * updated the Customer {@link CreditCardDetails}
  * </p>
  * @param inputTransactionDetails Credit Card transaction details
  * @return Transaction details binded in {@link CompletableFuture} as it is
  *         Asynchronous method
  */
 @Transactional
 public CompletableFuture<TransactionInputDetails> submitCreditCardTransaction(
   TransactionInputDetails inputTransactionDetails) {
  TransactionDetails transactionDetails = helper
    .generateTransactionDetails(inputTransactionDetails);
  validator.validateCreditCardTransaction(inputTransactionDetails);
  inputTransactionDetails
    .setTransactionTime(transactionDetails.getTransactionTime());
  helper.generateTransactionDetailsWithStatus(inputTransactionDetails,
    transactionDetails);
  repository.save(transactionDetails);

  if (Constants.TRANSACTION_STATUS_SUCCESS
   ...
   if (Constants.TRANSACTION_TYPE_DEBIT
    ...
    outStandingAmount = inputTransactionDetails.getCreditCardDetails()
      .getOutStandingAmount().add(inputTransactionDetails.getAmount());
   } else if (Constants.TRANSACTION_TYPE_CREDIT
     .equals(transactionDetails.getTransactionType())) {
    outStandingAmount = inputTransactionDetails.getCreditCardDetails()
      .getOutStandingAmount().subtract(inputTransactionDetails.getAmount());
   }
  ...
  * @return {@link CardRewardPoints} of a Credit card is binded in
  *         {@link ResponseEntity}
  * @throws DataNotFound When no Reward points found for a Credit Card
  */
 public ResponseEntity<ServiceResponse> getRewardPointsForCard(Long cardId)
   throws DataNotFound {
  Optional<CardRewardPoints> cardRewardPoints = rewardPointsRepository
    .findByCardId(cardId);
  ...
  return Utility.getResponseEntity(cardRewardPoints.get(), HttpStatus.FOUND,
    MessageConstants.DATA_FOUND);
 }

 /**
  * This method is used to search for Credit Card transaction based on search
  * criteria
  * @param searchQuery Search criteria on Credit Card to find the
  *                    {@link TransactionDetails}
  * @return List of {@link TransactionDetails} binded in
  *         {@link TransactionSearchResponse}
  */
 public TransactionSearchResponse getStatementPerCard(
